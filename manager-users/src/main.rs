use lib_protos::{users_manager_services_server::UsersManagerServicesServer, FILE_DESCRIPTOR_SET};
...
async fn main() -> Result<(), Box<dyn StdError>> {
    use api::users::UsersServiceHandler;
	...
	tracing::info!("Starting server on {}", CONFIG.get_server_config().get_socket_address());
    ...
    Ok(Server::builder()
        .add_service(UsersManagerServicesServer::new(UsersServiceHandler::new(
            conn_pool,
        )))
